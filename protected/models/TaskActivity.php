...
/**
 * This is the model class for table "{{dta_task_activity}}".
 *
 * The followings are the available columns in table '{{dta_task_activity}}':
...
class TaskActivity extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('task_id, by_user_id, activity_type ', 'required'),
			array('task_id, by_user_id, activity_type', 'length', 'max'=>20),
			array('activity_subtype', 'length', 'max'=>100),
			array('comments', 'length', 'max'=>1000),
			array('source_app', 'length', 'max'=>10),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('activity_id, task_id, by_user_id, activity_type, activity_subtype, comments, created_at, source_app', 'safe', 'on'=>'search'),
		);
	}
	...
	public function attributeLabels()
	{
		return array(
			...
			'activity_type' => Yii::t('label_model', 'lbl_activity_type'),
			'activity_subtype' => Yii::t('label_model', 'lbl_activity_subtype'),
			'comments' => Yii::t('label_model', 'lbl_comments'),
