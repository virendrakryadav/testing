...
class HUserInfoForm extends CFormModel {
	 ...
	 * @access public
	 */
	public function __construct($model,$modelContact, $emailAtt, $nameAtt = null, $scenario = 'both')
	{
		if(empty($emailAtt))
			throw new CException('$emailAtt can not be empty! Please specify email attribute name.');

		$this->email = $modelContact->$emailAtt;
		...
		$this->_model = $model;
                $this->_modelContact = $modelContact;
	...
	{
		return array(
			'email'=>$this->_modelContact->getAttributeLabel($this->emailAtt),
	...
	public function validateUser()
	{
		...
		$this->_model->clearErrors();
                $this->_modelContact->clearErrors();

		$user = $this->_model;
                $userContact = $this->_modelContact;
		...
		if($emailAtt)
		{
			$userContact->$emailAtt = $this->email;
			$attributes[] = $emailAtt;
			foreach ($userContact->getValidators($emailAtt) as $validator)
