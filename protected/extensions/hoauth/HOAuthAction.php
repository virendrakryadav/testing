...
class HOAuthAction extends CAction
{
	 ...
	 */
	public $model = 'User';
        public $modelContact = 'UserContact';
	 ...
	 */
	public $scenario = 'insertbysocial';
	 ...
	 */
	public $userIdentityClass = 'FrontUserIdentity';
	...
	protected function oAuth( $provider )
	{
		try{
			...
			else 
			{
					...
					if($this->useYiiUser)
						$user = User::model()->findByPk($oAuth->user_id);
					else
                                        {
                                            ...
                                            $userContact = call_user_func(array($this->modelContact, 'model'))->findByAttributes(array('user_id' => $oAuth->user_id , Globals::FLD_NAME_IS_PRIMARY => Globals::DEFAULT_VAL_CONTACT_IS_PRIMARY , Globals::FLD_NAME_CONTACT_TYPE => Globals::DEFAULT_VAL_CONTACT_TYPE));
                                        }
				}
				...
				if(!$oAuth->isBond || !$user)
				{
					if(!empty($userProfile->emailVerified))
					{
						...
						if($this->useYiiUser)
						...
						else
                                                {
                                                    ...
                                                    $userContact = call_user_func(array($this->modelContact, 'model'))->findByAttributes(array('contact_id' => $userProfile->emailVerified , Globals::FLD_NAME_CONTACT_TYPE => Globals::DEFAULT_VAL_CONTACT_TYPE));
                                                }
					...
					if(!isset($user))
					{
						...
						$user = new $this->model($this->scenario);
                                                $userContact = new $this->modelContact($this->scenario);
						$newUser = true;
					}

					if($this->alwaysCheckPass || $user->isNewRecord)
						if(method_exists($this->controller, 'hoauthProcessUser'))
							$user = $this->controller->hoauthProcessUser($user, $newUser , $userContact);
						else
							$user = $this->processUser($user, $userProfile , $userContact);
				}
				...
				if($accessCode === 1)
				{
                                    $user->email = $userContact->{Globals::FLD_NAME_CONTACT_ID};
	 ...
	 * @access protected
	 */
	protected function processUser($user, $userProfile , $userContact)
	{
		...
		if($user->isNewRecord)
			$this->populateModel($user, $userProfile , $userContact);
		...
		$form = new HUserInfoForm($user,$userContact, $this->_emailAttribute, $this->usernameAttribute );
...
//exit;
		...
		if($user->isNewRecord) 
		{
			if(!$user->save())
				throw new Exception("Error, while saving {$this->model} model:\n\n" . var_export($user->errors, true));
                        $userContact->{Globals::FLD_NAME_USER_ID}  = $user->primaryKey;
                        $userContact->{Globals::FLD_NAME_CONTACT_TYPE}  = Globals::DEFAULT_VAL_CONTACT_TYPE;
                        $userContact->{Globals::FLD_NAME_IS_PRIMARY}  = Globals::DEFAULT_VAL_CONTACT_IS_PRIMARY;
                        $userContact->{Globals::FLD_NAME_SOURCE_APP}  = Globals::DEFAULT_VAL_TASKER_SOURCE_APP_WEB;
                        
                        if(!$userContact->save())
                        throw new Exception("Error, while saving {$this->modelContact} model:\n\n" . var_export($user->errors, true));
			...
			if($this->useYiiUser)
			{
               ...
               // $user = array_merge($user ,$userContact);
		return $user;
	}
	 ...
	 * @access protected
	 */
	protected function populateModel($user, $profile , $userContact)
	{
//            echo '<pre>';
////            print_r($this->attributes);
//            print_r($profile);
////            print_r($userContact);print_r($user);
//            exit;
		foreach($this->attributes as $attribute => $pAtt)
		{
			if(in_array($pAtt, $this->_avaibleAtts))
			{
                           // echo $pAtt;
                            if( $pAtt == 'email')
                            {
                                $att2 = $profile->emailVerified;
                                        $userContact->$attribute = $att2;
                            }
                            else
                            {
                                switch($pAtt)
