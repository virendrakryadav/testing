			...
			if (isset($item['image']))
			{
				if (!isset($item['alt']))
					$item['alt'] = '';
                                ...
                                if (!isset($item['hrefId']))
					$item['hrefId'] = '';
                                
                                if (!isset($item['preImageHtml']))
					$item['preImageHtml'] = '';
                                
                                if (!isset($item['afterImageHtml']))
					$item['afterImageHtml'] = '';
				...
				if (!isset($item['imageOptions']))
					...
					$item['action'] = '#';
                                ...
                                if (!isset($item['beforeSend']))
					$item['beforeSend'] = '';
                                if (!isset($item['afterSend']))
					$item['afterSend'] = '';
                                if (!isset($item['success']))
					$item['success'] = '';
                                if (!isset($item['sendData']))
					$item['sendData'] = array();

                              echo   CHtml::ajaxLink($item['preImageHtml'].CHtml::image($item['image'], $item['alt'], $item['imageOptions']).$item['afterImageHtml'], Yii::app()->createUrl($item['action']), array(
                                'data'=>$item['sendData'],
                                'type'=>'POST',
                                'dataType' => 'json', 
                                'beforeSend' => 'function(){ '.$item['beforeSend'].' }',
                                'complete' => 'function(){ '.$item['afterSend'].' }',
                                'success' => 'function(data){'.$item['success'].' }'), 
                                array('id' => $item['hrefId'] ,'live'=>false));
