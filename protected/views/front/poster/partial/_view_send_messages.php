...
$img = CommonUtility::getThumbnailMediaURI($userid,Globals::IMAGE_THUMBNAIL_PROFILE_PIC_200);
    ...
    <?php 
        ...
        if($task->{Globals::FLD_NAME_CREATER_USER_ID} == Yii::app()->user->id )
        {
           // echo $form->hiddenField($message, Globals::FLD_NAME_IS_PUBLIC , array( 'value' => Globals::DEFAULT_VAL_MSG_IS_PUBLIC_ACTIVE ));   
            ...
            echo CHtml::hiddenField('replyMsg', '' ,array( 'id' => 'replyMsg')); 
        }
        else 
        {
...
<div class="messagesend">
    <div id="isPublicChk" style="visibility: hidden">
    ...
    if($task->{Globals::FLD_NAME_CREATER_USER_ID} == Yii::app()->user->id )
    {
        ...
        <?php
        echo $form->checkBox($message,Globals::FLD_NAME_IS_PUBLIC ,array('checked' => true) );
    }
    ?>
    </div>
<?php 
