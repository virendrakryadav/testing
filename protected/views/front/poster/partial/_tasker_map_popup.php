...
<div class="map-invite float-shadow " id="takerInfoOwterOnMap<?php echo $data->{Globals::FLD_NAME_USER_ID} ?>" >
...
<div class="invite-row4"><button id="userInviteBtnOnMap<?php echo $data->{Globals::FLD_NAME_USER_ID} ?>" type="button" onclick="addTaskerToInvite('<?php echo $data->{Globals::FLD_NAME_USER_ID} ?>','<?php echo CommonUtility::getUserFullName( $data->{Globals::FLD_NAME_USER_ID} ); ?>' , '<?php echo $img ?>' , 'invitedTaskersByMap','userInviteBtnOnMap' , 'taskerInvitedDivTitleOnMap' , 'takerInfoOwterOnMap' , 'invitedTaskersRemoveOnMap' )" class="btn-u btn-u-sm rounded btn-u-sea">Invite</button></div>
</div>
</div>